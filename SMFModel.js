class SMFModel extends Drawable {
    ...
    initBuffers() {
        const gl = this.gl;
        SMFModel.shaderProgram = initShaders(gl, "/vshader.glsl", "/fshader.glsl");

        SMFModel.positionBuffer = gl.createBuffer();
        gl.bindBuffer(gl.ARRAY_BUFFER, SMFModel.positionBuffer);
        gl.bufferData(gl.ARRAY_BUFFER, new Float32Array(this.vertices), gl.STATIC_DRAW);

        SMFModel.colorBuffer = gl.createBuffer();
        gl.bindBuffer(gl.ARRAY_BUFFER, SMFModel.colorBuffer);
    ...
    draw(camera) {
        ...
        gl.bindBuffer(gl.ARRAY_BUFFER, SMFModel.colorBuffer);
        let colorLoc = gl.getAttribLocation(SMFModel.shaderProgram, "aColor");
